...
def generate_calibration_board(
    ...
    for i in range(1024):
        ...
        for z in range(PrinterConfig.COLOR_LAYERS):
            full_matrix[z, py : py + pixels_per_block, px : px + pixels_per_block] = (
                stack[z]
            )
...
def get_top_1296_colors():
    ...
    for stack in itertools.product(range(6), repeat=5):
        ...
        srgb = sRGBColor(
            final_rgb[0] / 255.0, final_rgb[1] / 255.0, final_rgb[2] / 255.0
        )
        lab = convert_color(srgb, LabColor)

        candidates.append({"stack": stack, "lab": lab, "rgb": final_rgb})
...
def generate_smart_board(block_size_mm=5.0, gap_mm=0.8):
    ...
    print(
        f"[SMART] Board size: {board_w:.1f} x {board_h:.1f} mm (Grid: {total_dim}x{total_dim})"
    )
...
def generate_8color_board(page_index=0):
    ...
    try:
        ...
        print(
            f"[8COLOR] Surface black: {surface_black}/{len(all_stacks)} ({surface_black / len(all_stacks) * 100:.2f}%)"
        )
    ...
    full_matrix = np.full(
        (5 + int(PrinterConfig.BACKING_MM / 0.08), v_w, v_w), 0, dtype=int
    )
    ...
    for mid, col in conf["preview"].items():
        prev[full_matrix[0] == mid] = col[:3]
...
def generate_bw_calibration_board(
    block_size_mm: float = 5.0, gap_mm: float = 0.8, backing_color: str = "White"
):
    """Generate BW (2-color) calibration board with 8x8 border layout."""
