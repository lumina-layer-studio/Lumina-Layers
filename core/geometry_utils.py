...
def create_keychain_loop(
    width_mm, length_mm, hole_dia_mm, thickness_mm, attach_x_mm, attach_y_mm
):
    """
    ...
    """
    print(
        f"[GEOMETRY] Creating keychain loop: w={width_mm}, l={length_mm}, "
    ...
    bottom_faces = _connect_rings(
        bottom_outer_idx, bottom_hole_idx, vertices_arr, is_top=False
    )
    ...
    print(
        f"[GEOMETRY] Loop mesh created: {len(mesh.vertices)} vertices, {len(mesh.faces)} faces"
    )
