...
class VectorProcessor:
    ...
    def __init__(self, lut_path: str, color_mode: str):
        ...
        print(
            f"[VECTOR] ✅ Initialized with {len(self.img_processor.ref_stacks)} LUT colors"
        )
    ...
    def svg_to_mesh(
        ...
        print(
            f"[VECTOR] Found {len(shape_data)} valid shapes. Scale: {scale_factor:.4f}"
        )
        ...
        if color_replacements:
            try:
                ...
                replacement_manager = ColorReplacementManager.from_dict(
                    color_replacements
                )
        ...
        for i, current_color in enumerate(color_order):
            ...
            if is_small_feature:
                print(
                    f"  {hex_color}: area={area:.2f} sq units ⭐ SMALL FEATURE - PROTECTED"
                )
            ...
            for j in range(i):
                ...
                if earlier_geom is not None and not earlier_geom.is_empty:
                    try:
                        ...
                        final_geometries[earlier_color] = earlier_geom.difference(
                            merged
                        )
                    except Exception as e:
        ...
        print(
            f"[VECTOR] After Boolean operations: {len(final_geometries)} color regions"
        )
        ...
        for color_index, (svg_color, geometry) in enumerate(final_geometries.items()):
            ...
            if replacement_manager is not None:
                ...
                if replacement is not None:
                    _, rep_index = self.img_processor.kdtree.query(
        ...
        print(
            f"[VECTOR] Creating silhouette backing from {len(all_polygons)} polygons..."
        )
        ...
        for z in range(num_layers):
            ...
            for mat_id, geom_list in layer_map[z].items():
                ...
                if mat_id >= len(slot_names):
                    print(
                        f"[VECTOR] ⚠️ Skipping invalid mat_id {mat_id} (max {len(slot_names) - 1})"
                    )
                    continue
                ...
                for offset_idx, (geometry, color_idx) in enumerate(geom_list):
                    ...
                    new_meshes = self._extrude_geometry(
                        geometry, height=layer_h, z_offset=current_z, scale=scale_factor
                    )

                    if len(new_meshes) == 0:
                        geom_area = geometry.area
                        print(
                            f"[VECTOR] ⚠️ No meshes generated for mat_id={mat_id}, z={z}, area={geom_area:.2f}"
                        )
        ...
        if thickness_mm > 0:
            print(
                f"[VECTOR] Generating backing: {backing_layers} layers ({thickness_mm}mm)"
            )
            backing_meshes = []
            for i in range(backing_layers):
                backing_z = backing_z_start + (i * layer_h)
                backing_mesh_list = self._extrude_geometry(
                    silhouette, height=layer_h, z_offset=backing_z, scale=scale_factor
                )
    ...
    def _parse_svg(self, svg_path: str, target_width_mm: float):
        ...
        print(
            f"[VECTOR] Global Geometry Bounds: x={global_min_x:.1f}, y={global_min_y:.1f}, w={real_w:.1f}, h={real_h:.1f}"
        )
        ...
        for item in raw_shapes:
            ...
            shifted_poly = affinity.translate(
                original_poly, xoff=-global_min_x, yoff=-global_min_y
            )
